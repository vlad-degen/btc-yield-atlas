...
RPCS={'morph':'https://rpc-quicknode.morphl2.io','morph2':'https://rpc.morphl2.io','morph3':'https://morph.drpc.org','eth':'https://ethereum-rpc.publicnode.com','base':'https://base-rpc.publicnode.com','etha':'https://gateway.tenderly.co/public/mainnet','etha2':'https://eth-mainnet.public.blastapi.io','etha3':'https://eth.drpc.org'}
def k256(b):
...
def ca(to,sig,args='',block='latest',chain='morph'):
    return eth_call(chain,to,sel(sig)+args,block)
def dec_addr(h,i=0): return '0x'+(h[2:] if h.startswith('0x') else h)[64*i+24:64*(i+1)]
def dec_addr_array(h):
    h=h[2:]; off=int(h[:64],16)*2; n=int(h[off:off+64],16)
    return ['0x'+h[off+64+64*i+24:off+64+64*(i+1)] for i in range(n)]
